...
const Ultratrapeupdtae = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://api.homeessentialshive.co.uk/api/v1/auth/ultratapesingle/${params.id}`);
        ...
        fetchData();
            ...
            const updateuser = async (e)=>{
                ...
                try {
                    let result = await fetch(`https://api.homeessentialshive.co.uk/api/v1/auth/ultratapesingle/${params.id}` , {
        method: "put",
        body :formdata
