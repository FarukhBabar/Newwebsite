...
const GlassTablewearupdate = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://api.homeessentialshive.co.uk/api/v1/auth/glasstablewaersingle/${params.id}`);
    ...
    const updateUser = async (e) => {
        ...
        try {
            let response = await fetch(`https://api.homeessentialshive.co.uk/api/v1/auth/glasstablewaersingle/${params.id}`, {
                method: "PUT",
                body: formData
            });
